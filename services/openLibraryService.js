...
export const searchBooks = async (query, page = 1, limit = 10) => {
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/search.json?q=${encodeURIComponent(query)}&offset=${offset}&limit=${limit}`
    );
...
export const getAuthorDetails = async (authorId) => {
  try {
    ...
    return {
      ...
      photoUrl: author.photos && author.photos.length > 0
...
export const getBooksBySubject = async (subject, page = 1, limit = 10) => {
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/subjects/${encodeURIComponent(subject.toLowerCase())}.json?offset=${offset}&limit=${limit}`
    );
...
export const getBookContent = async (bookId) => {
  try {
    ...
    const isAvailable = availabilityData?.items?.[`ol${olid}w`]?.is_readable === true;
...
async function getBookExcerpt(bookId) {
  try {
    ...
    const previewText = bookDetails.description || 
...
function processBookContent(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if ((line.toLowerCase().startsWith('chapter') || 
        ...
        i > 0 && lines[i-1].trim() === '' && // Preceded by blank line
        (i+1 < lines.length && lines[i+1].trim() === '')) { // Followed by blank line
