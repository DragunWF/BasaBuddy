...
const CategoryItem = ({ icon, title, onPress }) => (
  <TouchableOpacity 
    ...
    onPress={onPress}
  >
    <View className="w-14 h-14 rounded-lg justify-center items-center" style={{ backgroundColor: icon.color }}>
      {icon.component}
    </View>
...
const BookItem = ({ book, onPress }) => (
  ...
  >
    ...
    <View className="flex-1">
      <Text className="font-semibold" numberOfLines={1}>{book.title}</Text>
      <Text className="text-xs text-gray-600" numberOfLines={1}>{book.author}</Text>
...
function ExploreScreen({ navigation }) {
  ...
  const categories = [
    ...
    { 
      ...
      title: "Business", 
      icon: { 
        component: <MaterialIcons name="attach-money" size={20} color="white" />,
        color: "#4CAF50"
      }
    },
    { 
      ...
      title: "Comic", 
      icon: { 
        component: <MaterialIcons name="bubble-chart" size={20} color="white" />,
        color: "#FFC107"
      }
    },
    { 
      ...
      <View className="flex-1">
        ...
        <View className="bg-[#FE9F1F] pt-10 pb-4 px-4">
          ...
          <View className="flex-row bg-white rounded-full px-2 items-center shadow-sm">
            ...
            <TouchableOpacity onPress={() => handleSearch(1, true)} className="p-2">
